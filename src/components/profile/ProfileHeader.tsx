...
export function ProfileHeader() {
  ...
  return (
    <div className="bg-yellow-400 px-4 sm:px-5 pt-8 sm:pt-10 pb-5 sm:pb-6 rounded-b-3xl text-yellow-950 shadow-sm shrink-0">
      <div className="flex items-center gap-2.5 sm:gap-3">
        <div className="relative shrink-0">
          <div className="w-16 h-16 sm:w-24 sm:h-24 bg-yellow-900/10 rounded-full flex items-center justify-center backdrop-blur-sm border-2 sm:border-4 border-yellow-900/20 overflow-hidden shadow-sm">
            {isUploading ? (
              <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-950 animate-spin" />
            ) : profile?.photoURL ? (
              <img src={profile.photoURL} alt={userName} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            ) : (
              <User className="w-8 h-8 sm:w-10 sm:h-10 text-yellow-950/70" />
            )}
          </div>
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={isUploading}
            className="absolute bottom-0 right-0 sm:bottom-1 sm:right-1 bg-white text-yellow-500 p-1.5 sm:p-2 rounded-full shadow-md hover:bg-gray-100 transition-colors z-10"
            title="Trocar Foto"
          >
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg sm:text-xl font-bold font-serif mb-1 line-clamp-1">{userName}</h1>
